...
// Function to parse a string of coordinates and create a vector of points
...
int main(int, char**){
    ...
    // for (const auto& path : nodedata) {
    //     std::cout << "Path Index: " << path.first << std::endl;
    ...
    // }
      ...
      // Build the graph
    Graph::AdjacencyList adjacencyList = inst.buildGraph(nodedata);

    // Print the adjacency list with distances rounded to two decimal places
    for (int i = 0; i < adjacencyList.size(); ++i) {
        std::cout << "Node " << i << ":";
        for (const auto& edge : adjacencyList.at(i)) {
            std::cout << " [Node " << edge.destination << ": "<< std::fixed << std::setprecision(2) << edge.weight << "]";
        }
        std::cout << std::endl;
    }
